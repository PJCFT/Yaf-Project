...
/*检查管理员是否登录
 * */
function checkVisitor_Login(){
    //开启session
    session_start();
    if(!isset($_SESSION['visitor']) || empty($_SESSION['visitor'])){
        return false;
    }
    return true;
}
...
function imgUpload($file){
    ...
    //上传目录
    $uploadPath = '../style/upload/';
    //上传目录访问url
    $uploadUrl = '/admin/style/upload/';
    //上传文件夹
    ...
    //url地址
    $imgUrl = 'http://'.$_SERVER['SERVER_NAME'] . $uploadUrl.$fileDir.$img;
